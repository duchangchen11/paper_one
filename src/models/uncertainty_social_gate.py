"""Uncertainty-aware social interaction gate for JAAD."""

import torch
from torch import nn


class UncertaintySocialGate(nn.Module):
    """Use target uncertainty to regulate the contribution of nearby pedestrians."""
    ...
    def forward(
        ...
        # Estimate uncertainty from an ungated target-social proposal. The
        # entropy then controls the final social contribution below.
        proposal_context = self.proposal_fusion(
            torch.cat([target_context, social_context], dim=-1)
        )
